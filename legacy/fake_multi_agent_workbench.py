...
from powershell_codex_viewer import CodexExecRunner, CodexRunState, resolve_codex_command
...
class MultiAgentCodexGui:
            ...
            with dg.HLayout(style={"flex_grow": 1, "min_height": 0, "gap": 10}):
                with dg.Sidebar(title="Orchestration", width=250):
                    ...
                    for role in self.roles:
                        dg.NavItem(role.name, page=role.name.lower(), badge=role.status)

                    dg.Label("Artifacts", class_="section", wrap=False, style={"margin_top": 14})
                    for role in self.roles:
                        dg.NavItem(role.artifact_name, page=role.artifact_name.lower())
    ...
    def handoff_instructions(self, role: AgentRole) -> str:
        markers = ", ".join(f"{target.name}: {self.handoff_marker(target.name)}" for target in self.roles if target.name != role.name)
        return (
    ...
    def extract_best_handoff(self, source_role: str, buffer: str) -> Handoff | None:
        ...
        for target in self.roles:
            if target.name == source_role:
                continue
            marker = self.handoff_marker(target.name)
    ...
    def validate_handoff_body(self, source_role: str, marker: str, body: str) -> bool:
        ...
        if len(trimmed) < HANDOFF_MIN_LENGTH:
            reason = f"too-short:{len(trimmed)}"
        elif len(trimmed) > HANDOFF_MAX_LENGTH:
            reason = f"too-long:{len(trimmed)}"
        elif trimmed.lower() == "and":
            reason = "template-fragment"
        elif "Direct message to the implementer:\nFirst stopping point:\nEvidence/tests expected:" in trimmed:
    ...
    def refresh_artifacts(self) -> None:
        ...
        output_by_name = {
            "PLAN.md": self.plan_output,
            "REVIEW.md": self.review_output,
            "IMPLEMENTATION_NOTES.md": self.implementation_output,
            "TEST_RESULTS.md": self.test_output,
        }
        for artifact_name, output in output_by_name.items():
            path = artifact_dir / artifact_name
            if path.exists():
                output.set_value(path.read_text(encoding="utf-8", errors="replace"))
            else:
